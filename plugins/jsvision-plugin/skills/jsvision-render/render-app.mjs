...
function resolvePackageImport(cwd, packageName) {
  ...
  while (true) {
    ...
    if (existsSync(manifestPath)) {
      ...
      const entry = manifest.exports?.['.']?.import ?? manifest.module;
      if (typeof entry !== 'string' || entry.includes('..')) {
        throw new Error(`${packageName} does not declare a safe ESM import entry`);
      }
  ...
  throw new Error(`cannot resolve ${packageName} from ${cwd}; install the latest stable package first`);
}
...
export async function renderModule(opts) {
  ...
  const uiEntry = resolvePackageImport(cwd, '@jsvision/ui');
  const { createEventLoop, resolveCapabilities } = await import(pathToFileURL(uiEntry).href);
