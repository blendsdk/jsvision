...
### 13. A centered dialog is clipped on a small terminal

**Symptom:** the dialog title, buttons, or bottom frame disappears at the minimum viewport.
**Cause:** `width`/`height` centers the dialog but does not guarantee it fits the desktop's usable
bounds; menu and status chrome already reduced that area.
**Fix:** size against `app.desktop.bounds` and use a scrolling or multi-column chooser when content
cannot fit: `height: Math.min(desiredHeight, app.desktop.bounds.height)`.

### 14. Cancel leaves a preview theme active

**Symptom:** closing a theme/settings dialog with Esc keeps temporary visual changes.
**Cause:** preview mutated application state without saving and restoring the committed value.
**Fix:** capture the previous value, preview reactively, commit only on OK, and restore on every
non-OK result. Dispose the preview root and remove the dialog in `finally`.

### 15. A generated theme has unreadable shortcuts

**Symptom:** ordinary text passes contrast checks but hotkeys disappear in selected menus, buttons,
tabs, or status items.
**Cause:** one accelerator token was reused on both a light surface and an accent container.
**Fix:** audit concrete roles and set selected/focused hotkeys to that role's on-color with targeted
`roleOverrides` or post-generation role copies. See `theming.md`.

### 16. A window shadow looks like disabled text

**Symptom:** the shadow is pale or changing it makes disabled menu/status text unexpectedly strong.
**Cause:** generated themes derive `shadow` from a disabled token, and current menu/status disabled
