...
```
packages/core/     Published @jsvision/core — zero-dep foundation engine: src/engine/** (capability · input · render · host · safety · color/theming) + bench/ + tests. Single entry src/engine/index.ts.
packages/ui/       @jsvision/ui — Turbo Vision-style widget framework (retained tree + signals) on core. src/{reactive,layout,view,event,app,desktop,window,menu,status,controls,scroll,list,dialog,table,tabs,feedback,date,color,surface,router,tree,editor,dropdown,terminal}/. Single entry src/index.ts. Private until release.
packages/web/      @jsvision/web — browser runtime: xterm.js host, buildBrowserCaps, mountApp, in-memory virtual fs, key-reclaim, outbound clipboard. Deps core+ui+files.
packages/files/    @jsvision/files — Turbo Vision file-system dialog family (FileDialog/ChDirDialog + openFile/errorBox) on ui. Private until release.
packages/theme-designer/  @jsvision/theme-designer — standalone TUI app to author core themes (headless model + view + host). Deps core+ui+files.
packages/examples/  @jsvision/examples — demos, capability probe, and the kitchen-sink showcase (Storybook-for-TUI). Imports every package by name.
packages/docs-site/  @jsvision/docs-site — VitePress site + live-example system + generated API reference (docs:api). Dev-only; joins verify's test+typecheck, build isolated as vp:build.
packages/spike-data-studio/  Throwaway feasibility spike (inert; skipped by verify). Delete after the Data Studio decision.
docs/              Monorepo docs — now only acceptance-gate.md (the gate oracle for gate.spec + gate.mjs; do not move).
scripts/           Root scripts: gate.mjs, check-no-native-deps.mjs, sync-versions.mjs.
codeops/           CodeOps nested layout (marker .codeops.yml): features/<slug>/, roadmaps, _archive/.
_archive/          Archived Ink/React prototype — reference only, not built.
.github/workflows/  CI (ci.yml: 3 OS × Node 20/22/24) + docs Pages deploy (docs.yml).
tsconfig.base.json · turbo.json · per-package vitest.config.ts — shared TS config, turbo pipeline, vitest unit+e2e. packages/*/dist/ + terminal-matrix.json are gitignored.
```

**Tests** live in each package's `test/` (never colocated): `*.spec.test.ts` (immutable spec oracle), `*.impl.test.ts` (internals/edges), `*.e2e.test.ts` (vitest `e2e` project). core imports source via `../src/engine/...`; examples import `@jsvision/core` by name.
