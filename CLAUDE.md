...
<!-- analyze_project: refreshed 2026-07-15 — added @jsvision/datagrid to Overview/Toolchain/Project structure (RD-06 filtering landed); all hand-authored sections preserved verbatim -->

## Overview

- **Type:** monorepo (library SDK) — published `@jsvision/core`; private-until-release `@jsvision/ui` (widgets), `@jsvision/web` (browser runtime), `@jsvision/files`, `@jsvision/datagrid` (editable enterprise data grid on `ui`), the `@jsvision/theme-designer` app, `@jsvision/examples`, and `@jsvision/docs-site`.
...
- **Repo shape:** **yarn 1.x + Turborepo monorepo** (`packages/*` workspaces). Public: `@jsvision/core` (published, zero runtime deps). Private-until-release: `@jsvision/ui` (widget framework), `@jsvision/web` (browser runtime), `@jsvision/files`, `@jsvision/datagrid` (editable data grid on `ui`), `@jsvision/theme-designer` (app), `@jsvision/docs-site` (VitePress site — joins `yarn verify`'s test+typecheck; its build is isolated via the `vp:build` script name), `@jsvision/examples` (demos + probe). Public packages share one lockstep version (`yarn sync-versions`; root `package.json#version` is the source of truth).
...
```
packages/core/       Published @jsvision/core — zero-dep foundation engine under src/engine/ (capability · input · render · host · safety · color) + bench/ + tests.
packages/ui/         @jsvision/ui (private) — Turbo Vision-style widget framework on core: reactive/layout/view/event/app-shell + widgets (controls, scroll/list/dialog, table, tabs, feedback, date, color, surface, tree, editor, dropdown, terminal); single barrel src/index.ts.
packages/web/        @jsvision/web (private) — browser runtime: createBrowserHost/mountApp, buildBrowserCaps, in-memory virtual FileSystem, key-reclaim, clipboard, ./browser-stubs subpath.
packages/files/      @jsvision/files (private) — Turbo Vision file-system dialog family on ui.
packages/datagrid/   @jsvision/datagrid (private-until-release) — editable, enterprise-class data grid on ui: typed column model + GridDataSource, pure sort/filter models, SortHeader (sort + funnel), virtual-scroll editable body + typed cell editors, filter/value-list popups; single barrel src/index.ts; kitchen-sink stories under test/.
packages/theme-designer/  Private app — standalone TUI for authoring core themes (deps core+ui+files; tsx start, no build).
packages/docs-site/  Private VitePress docs & showcase site (dev-only; joins verify's test+typecheck, build isolated via vp:build).
packages/examples/   Private @jsvision/examples — dev demos, capability-probe harness, kitchen-sink showcase, web-xterm browser dogfood.
packages/spike-data-studio/  Throwaway feasibility spike (inert; delete after the decision).
docs/                Monorepo docs — now only acceptance-gate.md (the go/no-go oracle).
scripts/             Root tooling: gate.mjs, check-no-native-deps.mjs, sync-versions.mjs.
.github/workflows/   ci.yml (3 OS × Node 22/24) + docs.yml (GitHub Pages).
codeops/             CodeOps nested layout — portfolio + features/ + _archive/ roadmaps, requirements, plans.
_archive/            Archived Ink/React prototype (reference only, not built).
Root config          tsconfig.base.json, turbo.json, per-package vitest.config.ts, CHANGELOG.md.
Tests                In each package's test/: *.spec.test.ts (immutable oracle) · *.impl.test.ts (edges) · *.e2e.test.ts (e2e project).
```
