...
codeops/         CodeOps nested layout (marker .codeops.yml: codeopsLayout=nested, rdIdScope=per-feature). 00-roadmap.md = portfolio roadmap; features/<slug>/ holds one feature-set's 00-roadmap.md + requirements/ + plans/<plan>/. Active feature: features/jsvision-ui/ (requirements RD-* + plans layout-engine, reactive-core, tui-ui, monorepo-restructure, view-group-spine). The completed RD-01…RD-10 foundation feature-set is archived under codeops/_archive/foundation/ (incl. its requirements/). Source-relative links from a plan reach the repo root via ../../../../../.
