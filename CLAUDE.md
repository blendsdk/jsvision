...
packages/ui/             @jsvision/ui — the Turbo Vision-style widget framework (retained tree + signals) on @jsvision/core. Single public entry src/index.ts re-exports each landed subsystem: src/layout/ (ADR-008 cell-native apportionment + flex track solver) and src/reactive/ (RD-01 reactive core — see below). private until first release; remaining subsystems land per plans/tui-ui/01-component-map.md.
