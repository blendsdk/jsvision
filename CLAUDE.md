...
<!-- analyze_project: refreshed 2026-07-16 — merged the datagrid + forms branches; Overview/Toolchain/Project-structure list both @jsvision/datagrid and @jsvision/forms. Hand-authored Prime-directive/Conventions/Git/Documentation/TV-fidelity/Kitchen-sink/Special-rules preserved verbatim. -->
...
- **Type:** monorepo (library SDK) — published `@jsvision/core`; private-until-release `@jsvision/ui` (widgets), `@jsvision/web` (browser runtime), `@jsvision/files`, `@jsvision/datagrid` (editable enterprise data grid on `ui`), `@jsvision/forms` (headless form store + Zod validation + opt-in async validation), the `@jsvision/theme-designer` app, `@jsvision/examples`, and `@jsvision/docs-site`.
...
- **Repo shape:** **yarn 1.x + Turborepo monorepo** (`packages/*` workspaces). Public: `@jsvision/core` (published, zero runtime deps). Private-until-release: `@jsvision/ui` (widget framework), `@jsvision/web` (browser runtime), `@jsvision/files`, `@jsvision/datagrid` (editable data grid on `ui`), `@jsvision/forms` (headless form store + Zod validation + opt-in async validation), `@jsvision/theme-designer` (app), `@jsvision/docs-site` (VitePress site — joins `yarn verify`'s test+typecheck; its build is isolated via the `vp:build` script name), `@jsvision/examples` (demos + probe). Public packages share one lockstep version (`yarn sync-package-versions`; root `package.json#version` is the source of truth).
...
packages/web/      @jsvision/web — browser runtime: xterm.js host + buildBrowserCaps + mountApp + in-memory virtual FS + key-reclaim + outbound clipboard; no backend. Single barrel src/index.ts.
...
packages/datagrid/ @jsvision/datagrid — editable, enterprise-class data grid on ui: typed column model + GridDataSource, pure sort/filter models, SortHeader (sort + funnel), virtual-scroll editable body + typed cell editors, filter/value-list popups + a filterPopup customization seam; single barrel src/index.ts; kitchen-sink stories under test/. Private until release.
