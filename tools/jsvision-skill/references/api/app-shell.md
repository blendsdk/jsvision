...
interface EventLoop {
  ...
  acquireCapture(view: View, onLost: PointerCaptureLostHandler): PointerCaptureLease;   // Capture pointer input for one gesture and receive synchronous notification when ownership ends. The returned lease is generation-bound, so delayed cleanup cannot release a replacement owner.
  ...
  notifyCaptureLost(): void;   // Notify the loop that the host lost pointer ownership or focus without a decoded focus report. The active lease, if any, is synchronously ended with the `host-lost` reason.
