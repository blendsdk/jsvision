...
- To drive it in a test: `loop.emitCommand('ok')`, then check the promise settled (or not, for a
  veto).
- For a desktop modal, add the dialog before `execView`, then remove it in `finally`. Save and restore
  temporary preview state on every non-OK result, and dispose external reactive roots there too.
