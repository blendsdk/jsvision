...
The engineering _underneath_ is genuinely excellent — a faithful Solid-style reactive core, a
...
But the **developer-facing surface has not caught up to the engine.** A modern developer _will_ fight
...
- mutate-`.layout`, there is no declarative authoring, the polished conveniences are missing or
  ...
  cheapest, highest-impact problem — the public JSDoc is written for the framework's _maintainers_, not
  its _users_. **The good news: almost every deduction is a thin-layer or documentation fix, not an
...
**Unweighted mean ≈ 6.7; experience-weighted ≈ 6.0** — because the dimensions a developer hits _first
and most often_ (composition, the reactive seam, and docs) are the weakest, while the 9-tier
...
**The central tension:** every one of those strengths lives _below the waterline_. The developer
experiences them passively. What they experience _actively_ — composing a screen, authoring a widget,
...
(`packages/examples/controls-demo/main.ts:82-109`). A custom widget is _always_ a `View` subclass with
a hand-painted `draw()` — the base class documents subclassing as "_the_ escape hatch"
...
- **Imperative retained-tree world:** `bounds`, `state.visible/disabled/focused`, `Group.children` are
  **plain mutable fields**. Mutating them does _nothing_ until you manually call
...
- `measure()` is _optional_, and an `auto` leaf with no `measure` **silently collapses to `{0,0}`** —
...
The primitives exist; the _one-liners a developer reaches for daily_ do not:

- **No `caps: 'auto'` default.** `caps` is `REQUIRED` on `ApplicationOptions`, `EventLoopOptions`, and
  `RenderRootOptions` (`packages/ui/src/app/application.ts:28`), and the _only_ source is
...
- **No first-class app-command handler.** To catch an app command the shell doesn't handle, you build
  an _invisible post-process `View`_: the demo's `CommandSink` sets `postProcess=true`,
...
- **No `messageBox()`/`confirm()`/`inputBox()`.** The nice `Dialog` class exists but the flagship demo
  bypasses it for the _raw_ path: manual size-clamp, **manual centering math**
  ...
  (`packages/examples/tvision-demo/main.ts:141-163`). That the _showcase_ demonstrates the heavy path
...
- **Window placement is raw pixel math.** `help.layout.rect = { x:1, y:1, width:30, height:14 }` and
  `help.number = 1` by hand (`main.ts:166-187`). `desktop.tile()`/`cascade()` help _after_ placement,
...
A UI developer cannot stay inside `@jsvision/ui`. "Hello world" forces imports from _two_ packages and
...
- `ButtonOptions.command`: _"Command emitted via `ev.emit` on activation **(PA-1)**."_ (`packages/ui/src/controls/button.ts:25`)
- `ListViewOptions.onSelect`: _"…`index` is DISPLAY order, `item` the `T` **(PF-003)**."_ (`packages/ui/src/list/list-view.ts:35`)
- The `Button` class-level JSDoc — the first thing a hover shows — is ~13 lines of C++ port
  archaeology: _"Faithful to `TButton::drawState` (`tbutton.cpp:102-165`) … `cShadow = getColor(8)` =
  `cpButton[8]=0x0F` → cpGrayDialog slot 15 …"_ (`packages/ui/src/controls/button.ts:1-14`). None of it
...
The raw material for excellent docs is _already written_ — the ComboBox two-mode doc and the
ProgressBar variable-width caveat are genuinely helpful _once you filter the codes out_. This is a
...
- **`RadioGroup(labels, value)` and `CheckGroup(labels, value)` are positional and ship no options
  ...
  `MultiCheckGroup` in the same folder takes an options object _and_ exports `MultiCheckGroupOptions`.
...
`assertEssentials` produce an actionable message (_"Terminal does not meet the SDK essentials:
interactive TTY (raw-mode keyboard input)."_, `packages/core/src/engine/safety/errors.ts:37-40`) but
...
The codes are valuable _internally_ — keep them, but not in the sentence a consumer reads. Move
...
class CommandSink extends View {
  // an invisible post-process view...
  ...
  override onEvent(ev: DispatchEvent) {
    const e = ev.event;
    if (e.type === 'command' && this.handlers[e.command]) {
      this.handlers[e.command]();
      ev.handled = true;
    }
  }
}
...
export function messageBox(
  app: Application,
  o: { title: string; text: string; buttons?: 'ok' | 'okCancel' },
): Promise<'ok' | 'cancel'>;
export function confirm(app: Application, text: string): Promise<boolean>;
export function inputBox(
  app: Application,
  o: { title: string; label: string; value: Signal<string>; validator?: Validator },
): Promise<string | null>;
```
...
class HelpView extends View {
  ...
  override measure(a: Size2D) {
    return a;
  } // forget this -> invisible widget
...
const helpView = (lines: readonly string[]) =>
  view({
    draw: (ctx) => {
      ctx.fill(' ', ctx.color());
      lines.forEach((l, i) => ctx.text(0, i, l, ctx.color()));
    },
...
`run()` should call the gate that already exists so a non-TTY start throws the _actionable_ error
...
## Would a modern developer _fight_ this framework?
...
- **Day one:** they fight the `caps` prologue and the two-package import, then squint at IntelliSense
  full of `PA-1`/`tbutton.cpp` and wonder what a button _does_. (Proposals 1–2 erase this.)
...
- **Once past those:** they get a genuinely modern reactive model, two-way binding, a promise-based
  modal system, and a huge, faithful widget catalog — and they largely _stop_ fighting.
