...
> **Features**: 0 / 4 done · jsvision-forms reopened (first slice shipped; extended with showcase + follow-on slices) · split-panes executing (Phase 1/4 landed)
...
| split-panes | [→](features/split-panes/plans/split-panes/00-index.md) | 🔄 Executing — resizable split panes (GH #10): N panes + N−1 draggable splitters, row/col, nestable for grids. **Phase 1/4 landed & verified**: optional `min` on the flex layout engine (`TrackItem.min`/`Size.min` + private `apportionMin`, behind a no-min fast path proven byte-identical across 1641 ui tests); ST-1…ST-9 green. Remaining: theme roles (P2), the `SplitView` component (P3), kitchen-sink story (P4). Preflight ✅ PASSED — 6 findings (4 major, 2 minor) all resolved (PF-001 clamp-inversion, PF-002 stuck highlight, PF-003 live/commit callbacks, PF-004 signal-write normalization; specs ST-28…ST-31) | 13/43 tasks · 1/4 phases | 🔄 | 2026-07-17 |
