...
> **Status**: Active
> **Last Updated**: 2026-07-03 (jsvision-ui **RD-18 Feedback (ProgressBar + Spinner) → DRAFTED ✏️** — `make_requirements --continue` produced `requirements/RD-18-feedback.md` (AR-186…194, 14 AC), RD-12+ sibling 5/6. **GATE-1: TV has NO progress/gauge/spinner class** (whole-tree search returns nothing) → two documented new components; pieces grounded in Unicode Block Elements + CP437 shade (bar), the shipped `fallbackGlyph` path (ASCII fallback) + `RuntimeAdapter.setTimer`/`clearTimer` seam (optional timer). 4 user decisions: both determinate bar + indeterminate spinner (no marquee, AR-187); smooth sub-cell fill + ASCII fallback (AR-189); caller-driven pure `Spinner` + optional `runSpinner` helper (AR-190); presets `dots`/`line`/`blocks`, default `dots`→`line` fallback (AR-191). New `src/feedback/`; kitchen-sink stories + `demo:feedback`. Next: `preflight RD-18` (optional) → `make_plan RD-18`. Prior: jsvision-ui **RD-17 Tabs → DONE ✅** — `exec_plan tabs` complete: 4 phases / **21 tasks** spec-first (ST-1…ST-38 ↔ AC-1…AC-15 + impl). `packages/ui/src/tabs/` — `TabView extends Group` (eager pages via a keyed `For`, one visible via a **`state.visible` flip — no mount/dispose**, PF-001; read/render-time clamp + snap, PF-003; Ctrl+PageUp/Down + Alt-hotkey in `preProcess` scoped by `isWithin(getFocused, this)`, PF-002; `select`/`next`/`prev`/`closeTab` + `onChange`) + `TabStrip extends View` (folder-tab chrome, `stripGeometry` overflow auto-scroll, `hitStrip`, `←→`). GATE-1 pinned 3 `tab*` bytes by gray-dialog analogy (`0x7F`/`0x70`/`0x78`) + decoded the 4 tees; GATE-2 AFTER-diff ✅. Inner `col` container keeps `TabView.layout` free (DataGrid PF-101 idiom). Only additive surface = **3 core `tab*` roles**. Kitchen-sink `containers/tabs` story + `demo:tabs` + `tabs-demo.e2e`. Full `yarn verify`/`test:e2e`/`check:deps` green; jsvision-ui now **14/18**. Prior: jsvision-ui **RD-17 Tabs → PLAN PREFLIGHTED 🔬** — `preflight tabs` PASSED (0🔴/2🟠/1🟡/1🔵, all Option A): **PF-001** page-switch "via `Show`" vs "no mount/dispose" was contradictory (`Show` disposes the inactive page, losing state) → reactive **`state.visible` binding**; **PF-002** "global from anywhere inside the `TabView`" fired every tab view in scope (`dispatch.ts:154`) → scoped by **`isWithin(ev.getFocused(), this)`** + new ST-37/38; **PF-003** read-time clamp; **PF-004** AC label. Report `plans/tabs/00-preflight-report.md`; 8 plan docs edited. Next: `exec_plan tabs`. Prior: jsvision-ui **RD-17 Tabs → PLAN CREATED 📋** — `make_plan RD-17` produced `plans/tabs/` (9 docs, 21 tasks / 4 phases, spec-first ST-1…ST-36 ↔ AC-1…AC-15). **Zero-Ambiguity Gate PASSED** (PA-1…PA-7; 4 user decisions all took the recommendation): all three Should-Haves in scope · local `src/tabs/` glyph set (frame line/corner + added tees `┬ ┴ ├ ┤`) · 3 roles `tabActive`/`tabInactive`/`tabDisabled` · renderer `tab-strip.ts`. GATE-1 BEFORE/AFTER fidelity tasks pin the 3 `tab*` bytes via `cpAppColor` + decode the 4 tees; AC-4 feeds real decoder bytes (`CSI 6;5~`/`CSI 5;5~`), Ctrl+Tab asserted inert without the keyboard protocol; kitchen-sink `containers/tabs` story + `demo:tabs` scheduled. Next: `exec_plan tabs`. Prior: jsvision-ui **RD-17 Tabs → PREFLIGHTED 🔎** (`preflight RD-17` PASSED, 0🔴/1🟠/1🟡/1🔵 all Option A; PF-001 Ctrl+Tab→Ctrl+PageUp/Down, PF-002 add tees, PF-003 story id). Prior: jsvision-ui **RD-17 Tabs → DRAFTED ✏️** — `make_requirements --continue` produced `requirements/RD-17-tabs.md` (AR-172…182, 15 AC), RD-12+ set sibling 4/6, first **Later**-phase RD. **GATE-1: Turbo Vision has NO tab/notebook/tabstrip class** (whole-tree search — only `TRefTable` + the unrelated `TTable` demo view) → RD-17 is a documented **new** component, its pieces grounded in shipped TV facilities (frame glyphs, `parseTilde` hotkeys, disabled greying, `cpAppColor` active/inactive @ plan GATE-1). Self-contained `TabView extends Group` = folder-tab box-drawing strip (top, `┬`-notched) + bordered content region, one page via `Show`; MVP = all four (disabled · `~X~` hotkeys · `◄►` overflow · closeable/dynamic); `active: Signal<number>` + `tabs: Signal<Tab[]>`; Ctrl+Tab/`←→`/Alt-hotkey. Next: `make_plan RD-17`. Prior: jsvision-ui **RD-16 Table/DataGrid → DONE ✅** — `exec_plan table` complete: `DataGrid<T>` multi-column table (TV `TListViewer`-derived, GATE-1 decode + GATE-2 diff), 4 phases / 28 tasks spec-first (ST-1…ST-24 + impl); `packages/ui/src/table/` (`columns.ts`/`grid-rows.ts`/`data-grid.ts`) + 1 additive `tableHeader` core role + kitchen-sink `data-grid` story + `demo:table` + `table-demo.e2e`; PF-101 width-matched fr-bands inside an inner `col` container; full `yarn verify`/`test:e2e`/`check:deps`/`lint` green. jsvision-ui now 13/18. Prior: **RD-15 Tree → DONE ✅** — `exec_plan tree` complete: `Tree<T>` expandable outline (TV `TOutlineViewer`), 4 phases / 35 tasks spec-first (ST-1…ST-24), TV GATE-1 decode + GATE-2 diff; `packages/ui/src/tree/` + 4 additive `cpOutlineViewer` roles + kitchen-sink story + `demo:tree`; runtime PA-16 (blue-window outline colours, since gray-dialog degenerates Normal==Focus) + PA-17 (marker `expanded?'─':'+'`); full `yarn verify`/`test:e2e`/`check:deps`/`lint` green. jsvision-ui now 12/18. Prior: **RD-14 Input dropdowns → DONE ✅** — `exec_plan input-dropdowns` complete: `History` + `ComboBox<T>` on a shared anchored popup (5 phases / 47 tasks spec-first, TV GATE-1/2, runtime PA-16/PA-17); full `yarn verify` + `test:e2e` + `check:deps` green, kitchen-sink stories + `demo:dropdowns` shipped. Prior: **RD-13 runtime-hardening → DONE ✅** — all 10 phases / 120 tasks complete, final full gate PASSED; remediated the five-agent audit backlog spec-first with TV GATE decodes. Also today: the **RD-12+ high-value-controls set** was opened via `make_requirements` — sliced into 6 sibling RDs by mechanism (AR-125…129), **RD-14 (Input dropdowns) + RD-15 (Tree) + RD-16 (Table/DataGrid) drafted ✏️** (AR-130…161, GATE-1 TV decodes) — the **MVP set (RD-14/15/16) is now fully drafted**; RD-17…19 queued. And a new feature-set **bun-runtime** — RD-01 drafted ✏️, Zero-Ambiguity Gate PASSED AR-1…AR-10)
> **Features**: 0 / 2 done
...
|---------|---------|---------------|----------|--------|--------------|
| jsvision-ui | [→](features/jsvision-ui/00-roadmap.md) | 14 ✅ Done (RD-01…RD-07, RD-10, RD-11, RD-13, RD-14, RD-15, RD-16, RD-17) · RD-18 ✏️ Drafted · Backlog (RD-19, RD-08/09) | 14 / 18 done | 🔄 | 2026-07-03 |
