...
- [ ] **FR-4 — The keep-absolute boundary is honored.** The following stay absolutely placed and MUST NOT be flexed: window/desktop/drag-resize gestures; cursor/caret-anchored popups (menu, dropdown, `replacePrompt`'s outer frame); measure-anchored extents (datagrid `personalize`, `filter-popup`, `overlay`, `quick-filter reposition`); the `theme-designer/gallery.ts` scatter scene; the movable-window desktop apps; the polar `analog-clock`; and `keyboard-mouse-playground` (raw `ScreenBuffer`, no view tree). The base `Window` has no separable inner flex shell and is not split. **The application overlay (`app/application.ts`) is also keep-absolute** — see the hidden-host exclusion below. [AR-8]

  > **Hidden-host exclusion (recorded 2026-07-19, from the T-AO1 attempt).** A view that is
  > `state.visible = false` until it is used CANNOT source its geometry from the layout pass:
  > `render-root.ts:174` skips invisible children entirely (`display:none` semantics), so a `'fill'`
  > child that is hidden never receives solved `bounds`. The app overlay is exactly this shape — it
  > stays hidden so it neither paints nor swallows clicks until a popup mounts — while popup
  > placement reads its extent *at open time*, before it becomes visible. An explicit
  > `position:'absolute'` rect is set eagerly at construction and is therefore readable regardless of
  > visibility; that is load-bearing, not vestigial. Converting it broke popup clamping in
  > `menu/controller.ts` and, silently, in `dropdown/popup.ts`. Restoring correctness required *adding*
  > a fallback-chain helper to buy a 2-line deletion — a net machinery increase, which inverts FR-5.
  > **Do not re-attempt.** The four catcher overlays mounted *into* a host are unaffected: they are
  > visible whenever they are mounted, so their `cover()` conversions stand.
- [ ] **FR-5 — Machinery is deleted, not ported.** The conversion MUST remove, not re-express:
  ...
  - the manual full-viewport resize re-anchoring on the 4 catcher/host overlays, replaced by `cover()` (the app overlay is excluded per the FR-4 hidden-host exclusion and keeps its rect + re-anchor). [AR-10]
...
|------|---------|-----------------|-------------|
| **Tier 0 — parity-safe** | base `Dialog` `center(this)/at(this)`; 4 `cover()` overlays (`menu/controller.ts:230/286`, `dropdown/popup.ts:250`) — the app overlay is excluded per the FR-4 hidden-host exclusion; `formDialog` body `cover(body)`; walkthrough demos → `cover()/center()`; `demo-shell` inner → `center()` | none | **none** — centering/resize tests survive as parity witnesses |
...
6. [ ] The local `at`/`tv`/`place`/`centerX`/`PAIR_WIDTH`/`buttonRects` helpers are gone from the ui/editor/forms dialogs, and the 4 named catcher overlays use `cover()` with no manual resize re-anchoring; the app overlay retains its absolute rect + re-anchor per the FR-4 hidden-host exclusion. (FR-5)
