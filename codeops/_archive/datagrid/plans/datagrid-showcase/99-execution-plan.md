...
> **Last Updated**: 2026-07-15 19:02
> **Progress**: 22/22 tasks (100%) — Phase 1 & 2 complete (full `yarn verify` green)
...
- [x] 2.1.1 Extend `datagrid-showcase.smoke.spec.test.ts` with ST-5 (all 7 categories present) + ST-7 (per-cluster counts: Foundation 5 · Editing 5 · Cell editors 9 · Formatting 8 · Sorting 5 · Filtering 6). Red (only the seed demo exists). ✅ (completed: 2026-07-15 19:02)
...
- [x] 2.2.1 **Foundation** — the remaining 4 demos: `value-format-parse`, `data-source`, `read-only`, `theming` (seed `sizing` already exists). Register each. ✅ (completed: 2026-07-15 19:02)
- [x] 2.2.2 **Editing** — 5 demos: `per-cell-edit`, `commit-veto`, `dirty-tracking`, `cursor-nav`, `overlay` (cursor/overlay shown through a live grid, PF-023). Register. ✅ (completed: 2026-07-15 19:02 — dirty via a slow async onCommit; cursor/overlay via hints)
- [x] 2.2.3 **Cell editors** — 9 demos, one per `CellEditorKind` (`text`/`integer`/`decimal`/`boolean`/`date`/`enum`/`lookup`+F4/`readonly`/`custom`). Register. ✅ (completed: 2026-07-15 19:02 — shared `buildEditorStory` helper)
- [x] 2.2.4 **Formatting** — 8 demos: `number`, `currency`, `percent`, `date`, `boolean`, `labels`, `parse-roundtrip` (+`PARSE_FAILED`), `render-style` (custom `render` + `cellStyle`). Register. ✅ (completed: 2026-07-15 19:02)
- [x] 2.2.5 **Sorting** — 5 demos: `single`, `multi`, `value-aware`, `collator`, `push-down` (over `spy-source`, echoes `setSort`). Register. ✅ (completed: 2026-07-15 19:02)
- [x] 2.2.6 **Filtering** — 6 demos: `quick-filter`, `condition-text`, `condition-num-date`, `value-list`, `n-of-m`, `push-down` (over `spy-source`, echoes `setFilter`). Register. ✅ (completed: 2026-07-15 19:02)

### Step 2.3: Governance + hardening + verify

**Reference**: `03-02 §Governance` · `07 §Walkthrough` · AR #9 / PF-022
- [x] 2.3.1 Reconcile `codeops/kitchen-sink-gate.md`: route datagrid component stories to this app; record that `kitchen-sink/stories/data-grid.story.ts` (ui read-only `DataGrid`, a different component) is intentionally retained. ✅ (completed: 2026-07-15 19:02)
- [x] 2.3.2 Confirm the walkthrough drives all 46 registry entries (38 shipped + 8 placeholders) and the smoke ST-5/ST-7 pass; fix any demo that clips text or fails to paint. Green. ✅ (completed: 2026-07-15 19:02 — 57 datagrid-showcase tests green)
- [x] 2.3.3 Full `yarn verify` green. Phase-2 commit. (Optional techdocs/analyze_project re-run per exec_plan post-hooks.) ✅ (completed: 2026-07-15 19:02 — turbo 26/26 successful)
