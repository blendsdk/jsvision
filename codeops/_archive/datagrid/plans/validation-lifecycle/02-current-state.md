...
| `grid.ts` crosses the `< 1300` guard | High (expected) | Low | AR-7: all logic in new modules; the added public surface (4 documented options + wiring) crosses 1299, so re-base `< 1300` → `< 1500` across all three guard tests with rationale — never by re-inlining (the plan's initial `< 1350` estimate proved low once every option carried its JSDoc; see AR-23) |
