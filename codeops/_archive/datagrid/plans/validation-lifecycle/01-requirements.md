...
| Module placement / line guard | New modules; `grid.ts` thin; guard re-based `< 1300` → `< 1500` (added public surface crosses 1299), never by re-inlining | AR-7 |
...
2. [ ] The `grid.ts` line guard is re-based `< 1300` → `< 1500` across all three guard tests with the
