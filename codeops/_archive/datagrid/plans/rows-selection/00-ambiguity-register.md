...
| RT-5 (Phase 3 fork) | The synthetic-prefix rendering approach was left open by `03-03` ("draw inset in the panel, OR a sibling band — pin down before task 3.2.2"). The panel-inset option would offset all data geometry in the heavily-tested `EditableGridRows` **and** `SortHeader` (high regression risk). | Chosen: **separate leading band views** — `SyntheticHeaderBand` (the tri-state box, one row) and `SyntheticBodyBand extends GridRows` (per-row checkbox + gutter). The body band extends `GridRows`, so it inherits the **identical** virtual-scroll from the shared `focused`/`display` → free row-alignment sync with the data panel, with no scroll-replication. `EditableGridRows`/`SortHeader` stay **untouched** (byte-identical no-prefix path). The bands are prepended as leading segments in `grid-panels.ts`; the data column cursor never enters them (separate views), so `←`/`Home` stop at the first data column (AR-5). |
| RT-6 (Phase 4, AR-6 follow-through) | `03-04` places `insertRow`/`deleteRows`/`duplicateRow` in `grid.ts`, but inlining their logic pushed `grid.ts` to **1213 lines** — over the Phase-2/3 runaway-growth guard (< 1200) whose recorded intent (RT-1) was to hold the CRUD wiring in a controller. The fork: bump the guard vs. extract a controller; and if extracting, fold into `grid-selection.ts` vs. a dedicated module. | Chosen: **extract a dedicated `RowMutations<T>` controller (`row-mutations.ts`)**, the twin of `GridSelection` — not folded into `grid-selection.ts` (selection ≠ CRUD; single-responsibility) and not inlined (would relax the guard I just set). `grid.ts` keeps the three public methods as **thin delegators** (their JSDoc is the public contract) and lands at **1198** (under the guard). The controller takes `{ source, display, selection, assignKey, warn }`; `deleteRows` calls `selection.prune`; `warn` is `devWarn` scoped to `duplicateRow` (no console sink duplicated). The `duplicateRow` clone runs inside a `try/catch` → `warn` + no-op on a non-cloneable row (no partial insert). No spec touched; the Phase-2 impl-test comment updated to name `row-mutations.ts`. |
