...
| AR-20 | Reorder-drag vs sort-on-down (**runtime**, Phase 5, user-decided 2026-07-16) | The grid sorts on a title mouse-**down** (an immutable RD-05 convention its down-only spec oracles depend on), but a title **drag** reorders — and the down fires before a drag is known. **User chose: revert the sort on drag.** The `SortHeader` fires `onHeaderClick` on a title down as today; when a reorder drag actually starts it fires a new `onReorderStart()`, and the container restores the `sortKeys` snapshot it took just before that click — so a drag reorders without a net sort. A plain click never fires `onReorderStart`, so it still sorts. Rejected: (b) leave the drag's sort (janky UX — dragging a header also sorts by it); (c) modifier-gated reorder (less discoverable, diverges from the plan's press-and-drag spec). | ✅ Resolved |
