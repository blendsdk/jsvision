...
| **ST-7** | 6 | For each of `boolean`/`date`/`enum`/`lookup`/`text`, `F2` (and separately `Enter`) mounts the type-appropriate widget and focus lands on **one pinned target**: for the **leaf** editors `text`/`boolean` `getFocused()` **is the mounted widget** (`Input`/`CheckGroup` are focusable leaves); for the **Group** editors `date`/`enum`/`lookup` `getFocused() === editor.input` (`DatePicker`/`ComboBox` are `Group`s whose only focusable descendant is `.input` — focusing the Group itself is a no-op, AR #14 runtime). No "either/or" — each kind asserts exactly one value. | `cell-editor.spec.test.ts` |
...
- **Group-editor focus target** — `ev.focusView(editor)` on a `Group` editor (`DatePicker`/`ComboBox`) is a
  no-op (leaf-only focus), so the editing controller focuses `editor.input` explicitly for Group editors and the
  widget itself for leaf editors (`Input`/`CheckGroup`). This is what lets a `date` cell commit (Enter reaches
  the editor host through the focused `.input`) and pins ST-7's target for `date`/`enum`/`lookup` (AR #14).
