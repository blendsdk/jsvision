...
| 14 | Behavior / Focus (runtime) | ST-7 pins `date` focus as "`getFocused()` is the mounted widget" (grouped with text/boolean). But `DatePicker` (like `ComboBox`) is a `Group` whose focus target is its `.input` child — empirically `focusView(datePicker)` is a no-op (`getFocused()===null`); only `focusView(datePicker.input)` lands focus. So the pinned ST-7 assertion is not achievable, and the date editor cannot commit (Enter can't reach the editor host) unless `.input` is focused. | (A) **Focus `.input` for Group editors** (`DatePicker`/`ComboBox`), focus the widget directly for leaf editors (`Input`/`CheckGroup`); correct ST-7 so `date` asserts `getFocused()===editor.input` (joining enum/lookup); text/boolean still assert the widget. Matches shipped `@jsvision/ui`; no ui change. · (B) Make the `editorHost` wrapper the single focus target for all kinds — uniform, but keystrokes no longer reach the field (breaks masked date typing + ComboBox type-ahead). | (A) Focus `.input` for Group editors; correct ST-7 for `date` — decided by the user via `AskUserQuestion` on 2026-07-13 (exec). Refines preflight PF-005. | ✅ Resolved (runtime) |
