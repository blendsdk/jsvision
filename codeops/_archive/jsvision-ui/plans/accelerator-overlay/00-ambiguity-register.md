...
| AR-2 | Reveal emphasis | UX/visual | underline / reverse / `[ ]` box / brighten-only | **Bold + underline** the existing accent glyph (`Attr.bold \| Attr.underline`) — no layout shift, no new theme role (box shifts layout; brighten too subtle; reverse noisy). **Revised 2026-07-07** from underline-only: after live use, underline alone reads too faintly, so `accentStyle` now ORs bold as well (still additive, still no core change) | ✅ Resolved |
...
| AR-18 | `setAcceleratorMode` visibility | Architecture (runtime) | keep the loop's mode setter private (03-02) vs expose a public seam | **Public `EventLoop.setAcceleratorMode(on)`** — the menu-open dismiss (AR-7) needs the app to reach it (the `MenuLoopSeam.dismissAccelerators` → `loop.setAcceleratorMode(false)` wiring), and it doubles as the AR-13 hold-Alt upgrade seam. The internal `applyAcceleratorMode` stays private; `setAcceleratorMode` just wraps it in `runTick`. Also: `makeDrawContext`'s new `revealAccelerators` param is **optional (default `false`)** so the surface.ts + existing test callers stay untouched (additive, NFR-2). | ✅ Resolved |

**Gate status:** every row ✅ Resolved with an explicit user decision; user confirmed the complete
register 2026-07-07. Zero deferred. **✅ GATE PASSED.** (AR-18 = execution-time runtime note.)
