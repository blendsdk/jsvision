...
| RT-5 | 5 | How do render specs inspect the composed frame (ST-05/06/14 assert specific cells)? | `RenderRoot` exposes a `buffer(): ScreenBuffer` accessor for the live compose target, alongside `mount`/`resize`/`flush`/`serialize`. | A genuine host-integration/test seam (read the composed frame) and the only no-mock way to assert per-cell composition; extends, doesn't change, the planned RenderRoot surface. |
