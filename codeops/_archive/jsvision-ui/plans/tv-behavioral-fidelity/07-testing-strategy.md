...
| **ST-03** | Status emit-on-release | down "File" + **up** over "File" ⇒ `file` once. down "File" + drag + up over "Edit" ⇒ **`edit`** once (the item under the release point, PA-10 — not `file`, not nothing). down + up released **off all items** ⇒ no emit. up over a **disabled** item ⇒ no emit. A bare down with no up ⇒ no emit | AC-3 · `tstatusl.cpp` `handleEvent` |
