...
> **Commit mode (this run):** _skip all, commit at end_ (user choice 2026-06-30). No per-phase
> commits; one end-of-plan commit prompt after Phase 5. All work is saved to disk.
...
- [x] T1.1 — Add `event.loop.spec.test.ts` (**ST-01** pure construct/dispatch, **ST-16** one flush/tick, **ST-17** resize→reflow+one frame, **ST-18** onIdle on drain, **ST-19** onEvent-throw isolated). (07 §spec; AC-1,16,17,18,19) ✅ (completed: 2026-06-30)
- [x] T1.2 — Run `yarn workspace @jsvision/ui test` → confirm loop specs **fail (RED)**; all RD-01/02/03 suites stay green. ✅ (completed: 2026-06-30 — RED: `Cannot find module '../src/event/index.js'`)

### Session 1B — Implementation (GREEN)
- [x] T1.3 ✅ (completed: 2026-06-30) — `view/types.ts`: declare `CommandEvent`/`AppEvent`/`DispatchEvent` (PA-8, AR-60). `view/view.ts`: add `focusable`/`preProcess`/`postProcess` (default false) + retype `onEvent(ev: DispatchEvent)`. `view/group.ts`: add `@internal current: View|null`. Re-export contract types via `view/index.ts`. **Spec-oracle adaptation (PF-008, user-approved):** the base `onEvent` narrowing makes the *direct call* `view.tree.spec.test.ts:94` (`v.onEvent({type:'key',value:'x'})`) a TS2345; type-adapt that call argument to a valid `DispatchEvent` (`{ event:{type:'key',…}, handled:false }`) — the ST-15 assertion ("stub changes no state") is preserved, not weakened. (03-01; AR-56,60; PA-2,8; PF-008)
- [x] T1.4 ✅ (completed: 2026-06-30) — `event/types.ts`: `EventLoop`/`EventLoopOptions`. `event/event-loop.ts`: `createEventLoop(viewport,opts)` — build `RenderRoot` with a **deferring** `schedule`; `mount`; the shared **`runTick(work)`** (do work → drain cascade → `onIdle` → one `flush`, re-entrant calls join the active tick, `draining` reset in `finally`); `dispatch` routes through `runTick`; `resize` (reflow + one `flush`); `onEvent`-error isolation via `logger`. `event/index.ts` barrel + `src/index.ts` re-exports. (03-01,03-04; AR-61,64,66; PA-7,11)
- [x] T1.5 ✅ (completed: 2026-06-30) — Run tests → loop specs **GREEN** (one flush/tick, resize, idle, error isolation); 147/147 ui tests pass, no RD-01/02/03 regressions.

### Session 1C — Impl tests & hardening
- [x] T1.6 ✅ (completed: 2026-06-30) — `event.loop.impl.test.ts` (deferring schedule never self-flushes; re-entrant `emitCommand` coalesces; `renderRoot` accessor; `onEvent` retype compiles vs `onEvent(_ev: unknown)` subclass). (07 §impl)
- [x] T1.7 ✅ (completed: 2026-06-30) — `yarn verify` + `lint` green; no file > 500 lines (largest event/ file: event-loop.ts @ 197). **/gitcm** — `feat(view): additive focusable/preProcess/postProcess + DispatchEvent contract` + `feat(event): EventLoop skeleton — builds RenderRoot, one frame per tick` (two commits, per scope). _(commit pending user decision)_
...
- [x] T2.1 ✅ (completed: 2026-06-30) — Add `event.dispatch.spec.test.ts` (**ST-02** 3-phase order + handled, **ST-09** command dispatch, **ST-10** enable/disable + unknown-enabled, **ST-11** keymap consume). (AC-2,9,10,11)
- [x] T2.2 ✅ (completed: 2026-06-30) — Run tests → dispatch specs **RED** (5/5 fail as expected).

### Session 2B — Implementation (GREEN)
- [x] T2.3 ✅ (completed: 2026-06-30) — `event/dispatch.ts`: `route(ev)` — keymap consume (PA-1) → mouse/wheel branch stub (Phase 4) → pre (root→down) / focused-chain / post sweeps with `handled` short-circuit; `deliver` wraps `onEvent` in try/catch→logger. (03-02; AR-51,66; PA-1,2)
- [x] T2.4 ✅ (completed: 2026-06-30) — `event/commands.ts`: registry (`Map` of explicit overrides; `isEnabled` defaults true, PA-3); `emit` (drop if disabled, else enqueue CommandEvent onto the tick); keymap glue (`keymap.lookup`→`emit`). Wire `emitCommand` (public) **through `runTick`** so a standalone call drains + flushes (PA-11), `enableCommand`/`isCommandEnabled`, + `route`'s key→command. (03-02; AR-52,62; PA-1,3,11)
- [x] T2.5 ✅ (completed: 2026-06-30) — Run tests → dispatch specs **GREEN** (5/5); full ui suite 156/156 green after fixing a pathological re-entrant impl-test handler (infinite cascade; now emits only on key events).

### Session 2C — Impl tests & hardening
- [x] T2.6 ✅ (completed: 2026-06-30) — `event.dispatch.impl.test.ts` (both-flag view in both sweeps; handled-in-pre skips focus+post; command handled in post; `opts.commands` seed; disabled bound key drops). (07 §impl)
- [x] T2.7 ✅ (completed: 2026-06-30) — `yarn verify` (8/8 tasks) + `lint` green; largest event/ file 235 lines. _(commit deferred — skip-all-commit-at-end mode)_
...
- [x] T3.1 ✅ (completed: 2026-06-30) — Add `event.focus.spec.test.ts` (**ST-03** current chain + getFocused + single focused, **ST-04** Tab/Shift-Tab wrap + skip **+ built-in `tab`/`shift+tab` KeyEvent moves focus & is not also plain-dispatched (PA-10)** — note ST-04's `shift+tab` uses a synthetic event at the RD-04 layer; it's producible end-to-end since core decodes backtab (PF-010/RT-1, resolved), **ST-05** predicate + subtree + Group-focusable, **ST-06** focus-flip invalidates exactly old+new, one frame). (AC-3,4,5,6; PA-10, PF-010)
- [x] T3.2 ✅ (completed: 2026-06-30) — Run tests → focus specs **RED** (6/6 fail).

### Session 3B — Implementation (GREEN)
- [x] T3.3 ✅ (completed: 2026-06-30) — `event/focus.ts`: `isFocusable`/`noBlockingAncestor`/`isFocusableContainer` (AR-56,65); `focusLeaf` (set current-chain + flip 2 flags + invalidate); `getFocused`; `focusView` (no-op if non-focusable, PA-5); `focusNext`/`focusPrev` (wrap + descend into Group, AR-57); save/restore via the persisted `current` pointers. (03-03; AR-48,56,57,65; PA-5)
- [x] T3.4 ✅ (completed: 2026-06-30) — Wire the loop's public focus methods through **`runTick`** (one frame each, PA-11); wire dispatch Phase-2 chain + the **built-in `tab`/`shift+tab`→`focus.next()`/`focus.prev()`** branch in `route` (consumed, keymap-bound `tab` overrides — PA-10) to the focus manager; run tests → focus specs **GREEN**.

### Session 3C — Impl tests & hardening
- [x] T3.5 ✅ (completed: 2026-06-30) — `event.focus.impl.test.ts` (save/restore on group re-entry; descend focuses current/first; `focusView` non-focusable no-op; `focusNext` zero-focusable no-op; hidden-ancestor blocks a focusable leaf). (07 §impl)
- [x] T3.6 ✅ (completed: 2026-06-30) — `yarn verify` (8/8) + `lint` green; focus.ts 166 lines. _(commit deferred — skip-all-commit-at-end mode)_
...
- [x] T4.1 ✅ (completed: 2026-06-30) — Add `event.mouse.spec.test.ts` (**ST-07** top-most-first hit-test + view-local + overlap-on-top, **ST-08** focus-on-click + empty-space steals-no-focus). (AC-7,8)
- [x] T4.2 ✅ (completed: 2026-06-30) — Run tests → mouse specs **RED** (3 fail; the empty-space oracle is vacuously green pre-impl, justified).

### Session 4B — Implementation (GREEN)
- [x] T4.3 ✅ (completed: 2026-06-30) — `event/hit-test.ts`: `hitTestRoute(ev)` — normalize (AR-63), `topMost` walk (reverse children, ancestor-clip via `intersect`, skip `!visible`/`disabled`, AR-65), `ev.local`, `focusOnClick` (climb to nearest focusable, AR-50,57). Wire `route`'s mouse/wheel branch to it. (03-03; AR-50,57,63,65)
- [x] T4.4 ✅ (completed: 2026-06-30) — Run tests → mouse specs **GREEN** (4/4).

### Session 4C — Impl tests & hardening
- [x] T4.5 ✅ (completed: 2026-06-30) — `event.mouse.impl.test.ts` (reverse-z overlap; hidden/disabled subtree skipped; `focusOnClick` climbs to ancestor; off-tree point → null no-op; wheel delivers without stealing focus). (07 §impl)
- [x] T4.6 ✅ (completed: 2026-06-30) — `yarn verify` (8/8) + `lint` green; hit-test.ts 115 lines. _(commit deferred — skip-all-commit-at-end mode)_
...
- [x] T5.1 ✅ (completed: 2026-06-30) — Add `event.modal.spec.test.ts` (**ST-12** execView resolves on endModal, **ST-13** input capture + outer inert + outside-click ignored, **ST-14** focus save/restore, **ST-15** nested LIFO). Add `event.packaging.spec.test.ts` (**ST-20** re-export shape + check:deps + bounded/no-injection). (AC-12,13,14,15,20)
- [x] T5.2 ✅ (completed: 2026-06-30) — Run tests → modal specs **RED** (4/4); packaging ST-20 green (surface already exported).

### Session 5B — Implementation (GREEN)
- [x] T5.3 ✅ (completed: 2026-06-30) — `event/modal.ts`: `ModalFrame`, `modalStack`, `execView<R>` (push + saveFocus + focus modal + capture), `endModal<R>` (pop LIFO + restore focus + resolve, explicit-only PA-4). Wire **both** loop `execView` **and** `endModal` **through `runTick`** (PA-11) — `execView` wraps its synchronous push+focus in the tick so opening a modal paints exactly one frame without relying on `serialize()` (PF-009), then returns the `Promise`; `endModal` likewise. Confine dispatch `scopeRoot` + hit-test `scopeRoot` to the top modal subtree, **including the Phase-2 focused-chain bubble clamp at `scopeRoot`** so the outer tree stays inert (AR-53, PA-6, PA-12). (03-04; AR-53; PA-4,6,11,12; PF-009)
- [x] T5.4 ✅ (completed: 2026-06-30) — Finalize `event/index.ts` + `src/index.ts` re-exports (`createEventLoop`/`EventLoop`/`CommandEvent`/`DispatchEvent`). `packages/examples/event-demo/main.ts` + `"demo:events"` script + `event-demo.e2e.test.ts`. (03-04; AR-59; PA-7,9)
- [x] T5.5 ✅ (completed: 2026-06-30) — Run tests → modal + packaging specs **GREEN** (6/6); `demo:events` prints the focus+command+modal walkthrough (resolves 'ok', exit 0).

### Session 5C — Final gate
- [x] T5.6 ✅ (completed: 2026-06-30) — `event.modal.impl.test.ts` (endModal empty-stack no-op; modal scope clamp excludes outer ancestor+sibling PA-12; savedFocus-no-longer-focusable skip; **`execView` paints exactly one frame on open** via the flush-spy, incl. a no-focusable-child modal — PF-009). Full gate green: `yarn verify` (8/8) + `yarn test:e2e` (event-demo + core) + `yarn workspace @jsvision/ui check:deps` + `yarn lint`; no dead code; largest `event/` file 227 lines. (07 §impl, §security)
- [x] T5.7 ✅ (completed: 2026-06-30) — Updated the feature roadmap RD-04 row → stage `Done` ✅ (cascaded to portfolio `codeops/00-roadmap.md`). _(end-of-plan commit deferred — skip-all-commit-at-end mode)_
...
- [x] 1B Impl (GREEN): T1.3–T1.5
- [x] 1C Impl tests & harden: T1.6–T1.7 ✅ commit

### Phase 2 — 3-phase dispatch + commands + keymap
- [x] 2A Spec (RED): T2.1–T2.2
- [x] 2B Impl (GREEN): T2.3–T2.5
- [x] 2C Impl tests & harden: T2.6–T2.7 ✅ commit
...
- [x] 4A Spec (RED): T4.1–T4.2
- [x] 4B Impl (GREEN): T4.3–T4.4
- [x] 4C Impl tests & harden: T4.5–T4.6 ✅ commit

### Phase 5 — Modality + packaging + demo + gate
- [x] 5A Spec (RED): T5.1–T5.2
- [x] 5B Impl (GREEN): T5.3–T5.5
- [x] 5C Final gate: T5.6–T5.7 ✅ commit + push
