...
> **Status**: Draft (RD-01…RD-06 + RD-10 + RD-11 + RD-13 shipped; RD-07 shipped; the high-value-control bucket (RD-12+) drafted through **RD-14/15/16/17/18** (Input dropdowns, Tree, Table/DataGrid, Tabs, Feedback); the **date & color set drafted — RD-20 (Date family — Calendar + DatePicker) + RD-21 (Color family — ColorSwatch + ColorPicker)**; **RD-19 (Surface family — Surface + SurfaceView) drafted**; **RD-09 (Files package `@jsvision/files` — FileDialog/FileList/FileInput/FileInfoPane/DirList/ChDirDialog) drafted**; **RD-08 (Editor family — Editor/Memo/EditWindow/Indicator/Terminal + `FileEditor` in `@jsvision/files`) drafted 2026-07-06** — the widget set is now fully drafted; **RD-22 (Theming — Fluent-inspired color theme system: aliases + `createTheme` + presets + hot-swap + designer) drafted 2026-07-08**)
> **Created**: 2026-06-29
...
| **RD-09** | [Files package `@jsvision/files`](RD-09-files-package.md) | **Phase R — Files** (component map §9): the fs-bound dialogs relocated to a **new `@jsvision/files` package** on `@jsvision/ui`. All six decoded faithfully — **`FileDialog`** (`tfildlg.cpp`), **`FileList`** (2-col, trailing-sep dirs, `..`-first sort), **`FileInput`** (mirrors the list), **`FileInfoPane`** (path + name/size/date/time, no attrs), **`DirList`** (`└─┬`/`├─`/`└─` tree), **`ChDirDialog`** (`tchdrdlg.cpp`). Injectable **`FileSystem` seam** (default `node:fs`, in-memory for tests) · **sync** reads · **cross-platform** paths · hidden-by-default · symlink-safe · faithful message-box errors · TV `*`/`?` wildcard. `private` until first release; 0-or-1 additive `fileInfo` role; kitchen-sink `Files` story + `demo:files` | RD-11, RD-06, RD-05, RD-14, core |
| **RD-08** | [Editor family](RD-08-editor-family.md) | **Phase 3 — Heavy** (component map §8): the gap-buffer **`Editor`** (decode of `TEditor` — UTF-16-unit gap buffer, faithful `formatLine` rendering `0x1E`/`0x71`, full WordStar keymap incl. Ctrl-K/Ctrl-Q prefixes, selection/mouse, literal search/replace via the ported `editorDialog` seam + shipped find/replace dialog builders, **modern bounded undo/redo stack** — the one behavior extension, AR-253), **`Memo`** (two-way `Signal<string>`, Tab-transparent), **`EditWindow`** (blue window: editor + `Indicator` + scrollbars at TV rects, min 24×6), **`Indicator`** (`line:col` + `═`↔`─` drag swap), **`Terminal`** (`src/terminal/` — the faithful 32000-byte ring log sink, `write()`/`writeLine()`), and **`FileEditor` in `@jsvision/files`** (load/save/`.bak`/save-prompts over the RD-09 `FileSystem` seam). Demo oracle: a live **`tvedit` clone** + headless `demo:editor` + kitchen-sink stories | RD-05, RD-11, RD-06/07, RD-04, RD-01/02/03, RD-09, core |
...
| **RD-22** | [Theming](RD-22-theming.md) | A Fluent-inspired, **TUI-sized color theme system** (AR-264…AR-283, preflight-clear): a tiered model — OKLab **ramp** → 16 curated semantic **aliases** (`ThemeColors`) → the existing ~70 control roles via one canonical `rolesFromAliases()`. Adds `createTheme(seeds)` (seed→ramp **and** discrete `overrides`), optional `ThemeRole.attrs` (dim/bold axis), lossless `serializeTheme`/`parseTheme`, curated **presets** (monochrome/turboVision(=default)/slate/nord/dracula/solarizedDark/gruvboxDark), runtime **hot-swap** (`RenderRoot.setTheme`/`Application.setTheme` → one recompose), a WCAG `contrastRatio` helper, and a live **`demo:themes`** designer + kitchen-sink `Theming` story. **Additive**: the flat `Theme` stays the widget contract; `defaultTheme` byte-unchanged. Core model + ui hot-swap + examples designer | RD-03, RD-05, RD-01/02/04, core |
