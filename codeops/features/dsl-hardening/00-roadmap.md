...
|----|-------|----|------|-------|--------|--------------|-----------------|
| dsl-hardening | Layout DSL hardening (S1/S2/S3/S5/S7 + `dsl/` module split + split-view min proof) | — (direct, GH #113) | [dsl-hardening](plans/dsl-hardening/00-index.md) | Executing | 🔄 | 2026-07-19 | Standalone plan (no RD), created from GH #113. 6 phases / 32 tasks, spec-first; Zero-Ambiguity Gate PASSED (14 ARs incl. runtime AR-14, 18 ST-cases). Preflighted 2026-07-18→19 (PF-001…007) — PASSED, all 7 resolved. **exec_plan in progress (`--auto-commit`): Phase 1 ✅ (`dsl/` split + packaging-oracle repath) · Phase 2 ✅ (S1 `grow` `min` + `Flex.grow` object form + S7 falsy children in col/row/stack; ST-3 corrected to a binding scenario, AR-14 runtime signed off). 14/32 tasks, full `yarn verify` green each phase.** S6 (`flex()`) + S8 (Group-subclass) deferred — no consumer. |
