...
- **Authority:** AI — delegated by `--auto-design`.
- **Eligibility:** Correcting the Phase 31 pre-implementation oracle's Markdown-link parser without
  changing a curriculum requirement, product behavior, or implementation scope.
- **Objective:** Make ST-41 recognize valid Markdown links whose labels wrap across source lines.
- **Decision:** Replace the single-line label matcher with a bracket-bounded matcher that accepts
  line breaks inside a Markdown link label while preserving the existing destination constraints.
  Record Phase 31's initial result as an oracle defect followed by 15 pre-existing passes; do not
  rewrite valid Guide prose merely to satisfy the faulty matcher.
- **Evidence:** `testing-headlessly.md` already links to both `/guide/application-shell` and
  `/guide/events-commands-and-keymaps`, but each label is wrapped by Prettier. The oracle used
  `[^\n]*?` for label content and therefore omitted both real links.
- **Rejected alternatives:** Reflowing the Guide links would make formatting an accidental test
  contract and could regress on the next formatter run. Weakening prerequisite-link coverage would
  remove the intended ST-41 evidence.
- **Strongest counterargument:** A broader matcher could consume unrelated Markdown. Bounding the
  label at the next closing bracket accepts wrapped text without crossing the link-label grammar.
- **Confidence:** High — the two reported missing edges are directly present in the committed
  source, and no implementation change is required.
- **Hardening:** Rerun the immutable oracle and docs-site typecheck after the parser correction.
- **Policy version:** 1.
- **Root invocation ID:** `exec-guides-20260729T185023Z`.
- **Reopen triggers:** A valid bracket-bounded multiline Markdown link is omitted, or the matcher
  consumes across a closing label bracket.

**AR-90 (runtime):**

- **Authority:** AI — delegated by `--auto-design`.
- **Eligibility:** Updating one superseded cross-phase navigation assertion after its planned target
  became a verified Complete course.
- **Objective:** Keep the In production course oracle aligned with the final all-Complete curriculum
  without weakening any production behavior or evidence requirement.
- **Decision:** Replace the Phase 28 assertion that forbade a link to the then-planned Complete
  application course with an assertion requiring that now-resolved link. Preserve every other In
  production course and authentic-substitute assertion unchanged.
- **Evidence:** Phase 28 correctly kept `/guide/complete-application` unlinked before the page
  existed. Phase 29 created, reviewed, verified, promoted, and published that course. Phase 31
  requires resolved onward links across the final curriculum.
- **Rejected alternatives:** Leaving the completed course unlinked would preserve stale prose and
  contradict ST-41/ST-42. Removing the related-course assertion would weaken navigation coverage.
- **Strongest counterargument:** Specification oracles should not follow implementation changes.
  This change follows the catalog's authorized stage transition and original no-dead-link rule, not
  an implementation convenience.
- **Confidence:** High — the target page, catalog entry, sidebar route, focused course tests,
  production docs build, and authoritative repository verification are already green.
- **Hardening:** Rerun the entire Guide specification suite after changing only the link polarity.
- **Policy version:** 1.
- **Root invocation ID:** `exec-guides-20260729T185023Z`.
- **Reopen triggers:** The Complete application course returns to Planned/Upgrade, its route is
  removed, or the related-course link stops resolving.

**AR-91 (quality review):**

- **Authority:** AI — delegated by `--auto-design`.
- **Eligibility:** Correcting Major weaknesses in the final curriculum integration oracle without
  changing the approved Guide curriculum or weakening a learning outcome.
- **Objective:** Make public-import and risk-boundary completion evidence fail when snippets name
  type-only runtime values or laboratories merely contain safety vocabulary without proving the
  behavior.
- **Decision:** Require every non-type snippet import to exist on the loaded runtime module, correct
  the two `EventLoop` teaching imports to use `type`, and replace lexical safety proxies with direct
  interactions against the registered async, filesystem, browser, untrusted-text, and capstone
  laboratories. Retain an expanded ambient-capability source audit only as a secondary defense.
- **Evidence:** The initial hardened import check exposed `EventLoop` as a type-only export in two
  Views & focus snippets. The registered laboratories expose deterministic counters and mounted
  state for stale suppression, authorization denial, virtual resources, sanitization, redaction,
  cancellation, exact cleanup, and post-disposal inertia.
- **Rejected alternatives:** Keeping declaration-text fallback would continue accepting nonexistent
  runtime exports. Adding more safety keywords would not prove behavior. Waiving either Major
  finding is forbidden by the automatic-design quality policy.
- **Strongest counterargument:** Direct curriculum integration tests overlap course-specific
  implementation coverage. The overlap is deliberate at the final boundary: it proves the
  registered laboratories compose into the safety contract rather than merely checking isolated
  fixture source.
- **Confidence:** High — the strengthened oracle failed on a real type/value defect, then passed
  only after the snippet was corrected; all five risk-bearing workflows are exercised through
  mounted registered examples.
- **Hardening:** Run the focused integration oracle and complete Guide inventory, then request one
  fix-scoped independent re-review before the final build and repository verification.
- **Policy version:** 1.
- **Root invocation ID:** `exec-guides-20260729T185023Z`.
