...
| ST-4 | Virtual FS drives real dialogs | A `FileList`/`FileDialog` from `@jsvision/files` over a virtual FS seeded `{ '/home/demo': { 'a.txt':'…', 'sub': { 'b.txt':'…' } } }` lists `a.txt` + `sub`; entering `sub` lists `b.txt`; **no `node:fs`** reachable from the browser entry (import-graph scan) | AC-4 | ✅ |
...
| ST-11 | Virtual FS contract | All 18 `FileSystem` methods behave per 03-03: `readDir` lists + sorts, `stat`/`lstat` equal, `readFile`/`writeFile` round-trip, `rename`/`unlink` mutate, missing path throws the dialog-compatible error, path ops are pure POSIX | AC-4, AC-9 | ✅ |
