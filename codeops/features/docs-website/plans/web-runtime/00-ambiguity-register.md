...
| 9 (runtime) | Testing/API | `KeyReclaimOptions` (03-04) lists only `also` + `isFocused`, but reclaim adds a capture-phase `keydown` listener on `document` — with no jsdom (AR-4) a test cannot dispatch a synthetic keydown against the real document. | (a) add an optional injectable `target?` event target (default the global `document`), matching the clipboard's injected-bridge and the host's injected-timer testability seams / (b) mutate `globalThis.document` in tests | **(a) Add `target?: KeyEventTarget` to `KeyReclaimOptions`.** Minimal additive surface, consistent with AR-4's hand-mocked-globals decision and the existing injectable seams; the default remains the global `document` (a no-op when absent). Decided during execution. | ✅ Resolved |
...
`@jsvision/files` `FileSystem` seam has 14 synchronous methods + a `sep` property).
