...
| RD-06 | API reference (TypeDoc) | [RD-06](requirements/RD-06-api-reference.md) | [api-reference](plans/api-reference/00-index.md) | Done | ✅ | 2026-07-11 | Phase C. **✅ DONE** (2026-07-11): a generated TypeDoc→md→VitePress API reference for the 4 public barrels (core `src/engine/index.ts`; ui/files/web `src/index.ts`, badged pre-release), **gitignored** `api/<pkg>/` regenerated before build; **bidirectional** component↔reference via a 29-row symbol↔page map (forward "API reference →" + injected "Documented in →"); coverage/leakage/symbol/links/determinism/search gates in `check-docs-build.mjs` (20/20), pure helpers in `yarn verify`; TypeDoc devDeps only (`check:deps` unaffected). **4 phases / 16 tasks, spec-first, all `[x]`.** Trio typedoc 0.28.20 · plugin-markdown 4.12.0 · vitepress-theme 1.1.3 (TS 5.9.3 peer-ok); per-package invocation for clean `/api/<pkg>/<kind>/<Symbol>` URLs (AR-22); package-qualified titles fix cross-package re-export duplicates (AR-23); `barrelExports` exactly == the generated set per package (core 152/ui 234/files 35/web 18; 0 missing, 0 leaked); determinism byte-identical. Zero-Ambiguity Gate 21/21 + runtime AR-22/23. Preflight ✅ PASSED (7 findings). **✅ clean-dist `yarn verify` 22/22 + API gate chain 20/20 + `check:deps` green.** Commits `d66f474` (P1) · `00c21e8` (P2) · P3/P4 pending. Depends RD-01. |
...
- 2026-07-11: **RD-06 (api-reference) → ✅ DONE** via `exec_plan`. **All 4 phases / 16 tasks `[x]`, spec-first.**
  A generated TypeDoc→md→VitePress API reference for the 4 public barrels, gitignored + regenerated before
  every build (`docs:api` → `gen-api.mjs`, one TypeDoc run **per package** for clean `/api/<pkg>/<kind>/<Symbol>`
  URLs, AR-22), with **bidirectional** component↔reference cross-links via a 29-row `src/api/api-map.mjs`
  (forward "API reference →" in each mapped component's `## Related` + an injected "Documented in →" back-link),
  and drift-gated: the pure `.mjs` helpers (`barrelExports`/`injectBackLink`/`validateApiMap`) are unit-spec'd
  in `yarn verify`, and 6 new `API-*` checks (coverage/leakage/symbol/links/determinism/search) in
  `check-docs-build.mjs` (now 20/20). **Runtime decisions:** AR-22 (per-package invocation), AR-23
  (package-qualified frontmatter titles so cross-package re-exports like `resolveCapabilities` in core+ui stay
  title-unique; `srcExclude` test/). `barrelExports` exactly equals the generated set per package (0 missing / 0
  leaked); determinism byte-identical. TypeDoc/plugins/`typescript` are docs-site devDeps only (`check:deps`
  green). **✅ clean-dist `yarn verify` 22/22 + API gate chain 20/20.** Cascaded to the portfolio row.
