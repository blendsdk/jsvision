...
| **ST-2** | a seeded tree with one undocumented class export and one drifted recipe snippet (via injected/temp fixtures) | `detectDrift()` returns exactly `[{kind:'undocumented-widget',name}, {kind:'snippet-drift',module}]` (order-independent) | SC-1, FR-1 |
...
| **ST-5** | a known widget name (e.g. `Button`) | `buildCatalogEntryRequest(name)` returns a request whose `user` text contains the widget's real JSDoc lead + `@example`, and whose `target.file` is the catalog under the correct `##` section; it invents no behavior | SC-3, FR-3, AR-14 |
| **ST-6** | an `undocumented-widget` finding + a **fake** `DraftClient` returning a fixed bullet | `fixUndocumentedWidgets([f], fake)` awaits `fake.draft` with the built request and makes no network call | SC-4, FR-3b, AR-10 |
| **ST-7** | the catalog text + a drafted bullet for `name` | `applyCatalogEntry(...)` inserts it under the right section, and `checkBarrelCoverage([name,...], result, denylist)` no longer reports `name` | SC-4, FR-3 |
