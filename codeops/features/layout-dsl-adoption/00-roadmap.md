...
| [#120](https://github.com/blendsdk/jsvision/issues/120) | **flex-eliminate FileDialog/ChDirDialog/errorBox + retire grow-dialog.ts** | Rebuild | 📋 | RD-01/02; Tier 2; deletes 2 source files. Prerequisite #122 ✅ (in PR #123). Plan [files-flex-elimination](plans/files-flex-elimination/00-index.md) created 2026-07-19 — 7 phases / 18 tasks, ✅ gate passed (15 AR items). Branch `feat/files-flex-elimination`. Scope note: spans ui + files (a `wrapText` export, AR-4), deviating from the issue's one-package ground rule |
