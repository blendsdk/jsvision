...
| [#116](https://github.com/blendsdk/jsvision/issues/116) | datagrid widgets (button-row / grid-lifecycle / popups) | Port | 📋 | behavior-preserving; out of divergence set. Plan [widget-flex-adoption](plans/widget-flex-adoption/00-index.md) 📋 created 2026-07-19 (combined with #109) — 48 conversions in 10 files. **Sweep corrected:** `grid-panels.ts` has 24 sites and **all** are pure flex (the "only 3 of 23 convertible" note was wrong; the file has no `position:'absolute'` at all). The deferred `cover()` cluster (AR-8) and 3 `at()` sites (AR-7) are pulled in; `filter-popup.ts:272` stays excluded as the #113 S6 self-config case |
