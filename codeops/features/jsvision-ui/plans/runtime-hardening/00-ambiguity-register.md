...
| PA-17 | Behavioral (HR-35) | Bare top-level menu `item` semantics | **TV-faithful emit+close** — selection of a submenu-less top-level command item emits its command and closes the bar; pinned by a GATE-1 decode of `tmenuview.cpp` (if TV disagrees, the C++ wins). Esc always closes regardless | ✅ Resolved |
