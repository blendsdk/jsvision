...
> **Status**: ✅ GATE PASSED — all 20 items resolved (AR-18/AR-19/AR-20 are exec_plan runtime decisions)
...
| 20 *(runtime — exec_plan)* | Cross-feature oracle conflict (AR-6 made real) | The cross-widget Editor paste fallback (R4/R12/AR-6; ST-23 targets a *bare* Editor, and 03-03 §Editor.3 specifies falling back when the clipboard editor is "absent **or** empty") supersedes the RD-08 oracle `editor.spec.test.ts > ST-17`, which asserts a bare Editor's paste is a no-op ("TV null-clipboard semantics", RD-08 PA-2) | (a) update the RD-08 ST-17 oracle's final clause to the new fallback behavior; (b) narrow the fallback to injected-but-empty clipboards only + change ST-23 to inject one (contradicts 03-03's "absent or empty" and splits no-clipboard vs empty-clipboard) | (a) Update the one RD-08 assertion (`bare.getText()` `'c'`→`'abc'`) + its comment/title to the deliberate global-clipboard supersession — **user-chosen**. Only editor regression; the fallback is exactly the plan's specified design. | ✅ Resolved |
