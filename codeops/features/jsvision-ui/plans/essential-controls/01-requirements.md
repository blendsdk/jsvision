...
- [ ] **`Button`** — `[ text ]` + block-glyph drop-shadow; activates on click / `Space` / `Alt-hotkey`; a `default` button activates on `Enter` if unconsumed (PA-7); emits a typed `command` via `ev.emit` and/or calls `onClick`; `disabled` greys + is inert; roles **reuse** existing `button` (TV "normal") + `buttonFocused` (TV "selected") and add `buttonDefault`/`buttonDisabled`/`buttonShortcut` — i.e. RD-06 AC's `buttonNormal`→`button`, `buttonSelected`→`buttonFocused` (PA-5). (AC-3, PA-1/PA-7/PA-8)
