...
> `{ key:'tab', shift:true }` is **not producible from real terminal input** — plain Tab works
> end-to-end, Shift-Tab does not. Decision: **keep the built-in arm** and **track a core RD-06
> follow-up** (decode `CSI Z` → `{ key:'tab', shift:true }`); until it lands, ST-04's shift+tab
> case is a **mechanism-level** (synthetic) oracle, not an end-to-end one. Recorded as **RT-1**
> below.
...
|------|-------|----------|----------|-----|
| RT-1 | Cross-layer (RD-06) | Real Shift-Tab/backtab (`ESC [ Z`) is dropped by the core decoder, so the PA-10 `shift+tab`→`focusPrev` arm can't fire from a live terminal. | **Track a core RD-06 follow-up** to decode `CSI Z` → `{ key:'tab', shift:true }` (add `0x5A`→tab+shift in `keys.ts`); RD-04 keeps the built-in arm unchanged; ST-04's shift+tab case stays a synthetic mechanism-level oracle until the follow-up lands. | Backtab is a near-universal terminal sequence; the gap is in core, not in RD-04's mechanism (PF-010). |
