...
> **Last Updated**: 2026-07-06 (**RD-08 preflight passed** — DEF-36 registered: hardware-caret shape (DECSCUSR), PF-004; §B RD-09 row corrected to SHIPPED). Earlier 2026-07-06: **RD-08 (Editor family) gate passed** — DEF-34 registered: editor right-click context menu / popup-at-point, AR-256; DEF-35 registered: regex search, AR-255. Earlier: 2026-07-05 (**RD-09 (Files package `@jsvision/files`) DRAFTED** — DEF-32 registered: async directory reads + loading state, AR-236; DEF-33 registered: extended glob matching, AR-243). Earlier: 2026-07-05 (**RD-19 (Surface) DRAFTED** — DEF-31 registered: interactive/bundled-chrome scrolling for `SurfaceView`, AR-228). Earlier: 2026-07-02 (**RD-07 SHIPPED** — DEF-01/02/03/19 → Shipped; DEF-25 registered (OSC-52 read); DEF-21/22 stay deferred. Earlier: DEF-23/24 registered — the `217f9ea` width-probe follow-ups; DEF-23 → the [glyph-auto-swap plan](../plans/glyph-auto-swap/00-index.md), DEF-24 stays deferred)
...
| **DEF-31** | **Interactive scrolling for `SurfaceView`** — a focusable variant with built-in keyboard/wheel scroll (arrows/PgUp/PgDn/Home/End/wheel move `delta`) **and** an auto `Scroller`/`ScrollBar` chrome wrapper (bars ↔ `delta`) | RD-19 · AR-228 | unassigned (on real need) | Deferred — RD-19 ships the **passive, faithful** `TSurfaceView` (caller drives `delta`; composable with an RD-11 `ScrollBar` bound to `delta`); the interactive/bundled-chrome variants are clean additive follow-ups beyond the TV decode |
| **DEF-32** | **Async directory reads + loading state** for `@jsvision/files` — a Promise-based `FileSystem.readDir`, a loading indicator while a slow/network dir resolves, and race handling | RD-09 · AR-236 | unassigned (on real need) | Deferred — v1 is **synchronous** + TV-faithful; the injectable `FileSystem` seam already isolates the read, so an async mode is a clean additive follow-up once a slow-fs need appears |
| **DEF-33** | **Extended glob matching** for `@jsvision/files` — `**`, brace `{a,b}`, char-class `[a-z]`, extended globbing — as a first-class wildcard mode | RD-09 · AR-243 | unassigned (on real need) | Deferred — v1 ships the **TV-faithful `*`/`?`** wildcard (parity); a richer glob mode (or a bundled matcher) is separable, and the Should-Have caller `filter(entry)` hook covers the interim need |
| **DEF-34** | **Right-click editor context menu** (`initContextMenu`/`popupMenu`, `teditor2.cpp:97-106`) — needs a popup-menu-at-point primitive (menus are MenuBar/overlay-hosted today) | RD-08 · AR-256 | a future menus RD | Deferred — revisit at the first component needing popup-at-point |
| **DEF-35** | **Regex search mode** for the editor find/replace — TV search is literal + case/whole-word flags (`teditor2.cpp:389-421`, `scan`/`iScan`) | RD-08 · AR-255 | unassigned (on real need) | Deferred — v1 ships the faithful literal search; `search()` is a clean seam for a regex mode |
| **DEF-36** | **Hardware-caret shape** (insert vs overwrite block/underline, DECSCUSR) for the editor's Ins toggle — needs an additive core `cursor` shape API + a shape field through `desiredCaret()`/`onCaret`/`run()` | RD-08 · preflight PF-004 | unassigned (on real need) | Deferred — the RD-07 caret seam (`desiredCaret(): Point \| null`) and core `render/cursor.ts` are position-only (core DECSCUSR itself deferred core-side) |
...
| Editor family (`Editor`/`Memo`/`EditWindow`/`Indicator`/`Terminal` + `FileEditor` in `@jsvision/files`) | **RD-08** | ✏️ [Drafted](RD-08-editor-family.md) (2026-07-06) |
| Files package (`FileDialog`/`FileList`/`DirList`/`ChDir`) | **RD-09** (`@jsvision/files`) | ✅ SHIPPED 2026-07-06 (8 phases / 43 tasks — see the roadmap RD-09 row) |
