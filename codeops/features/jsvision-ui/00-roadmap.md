...
> **Status**: In Progress
> **Created**: 2026-06-29
> **Last Updated**: 2026-07-02 (RD-13 **DONE** ✅ — [runtime-hardening](plans/runtime-hardening/00-index.md): all 10 phases / 120 tasks COMPLETE, spec-first per HR; final full gate PASSED. Remediated the 2026-07-02 five-agent audit backlog — 3 critical + 12 major + ~25 minor — with TV GATE-1/GATE-2 decodes for every fidelity item)
> **Progress**: 10 / 12 done (RD-01 ✅, RD-02 ✅, RD-03 ✅, RD-04 ✅, RD-05 ✅, RD-06 ✅, RD-07 ✅, RD-10 ✅, RD-11 ✅, RD-13 ✅)
...
| RD-12+ | High-value controls (sliced from the RD-07 bucket) | — | — | Backlog | ⬜ | 2026-07-01 | Phase 2. The compound/heavy controls split out of the RD-07 bucket per AR-115: `History` (Input dropdown) · `ComboBox` (Input + ListView) · `Tree` (OutlineViewer) · `Tabs` · `Table`/`DataGrid` (incl. RD-11's deferred multi-column `ListViewer` `numCols`, AR-104) · `ProgressBar`/`Spinner` · `Surface`. Cohesive sibling RDs to be drafted per mechanism (like RD-06→RD-11). Demo: **clone `tvdemo`** (north-star). |
...
- **Recommended next:** **`exec_plan essential-control-completions`** — execute spec-first (per component:
  spec oracles RED → implement → GREEN → impl tests, running the TV BEFORE-decode/AFTER-diff gate each); the
  plan is now preflighted (8 findings resolved). RD-11 and the new glyph-auto-swap plan also remain
  `exec_plan`-ready.
