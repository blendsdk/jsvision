...
| RD-15 | Tree — expandable outline | [RD-15](requirements/RD-15-tree.md) | — | RD Drafted | ✏️ | 2026-07-02 | **Drafted** ✏️ (`make_requirements`, RD-12+ set sibling 2/6, MVP phase AR-129). A virtual-scroll `Tree<T>` from TV `TOutlineViewer`/`TOutline`; GATE-1 decode done (`toutline.cpp` — `graphChars "\x20\xB3\xC3\xC0\xC4\xC4+\xC4"` = `│├└─`+`+`/`─`, no brackets, two-tone collapsed text `color>>8`; palette `cpOutlineViewer "\x6\x7\x3\x8"`). Concrete reactive `TreeNode<T>` (plain data + view-owned expand Set, AR-141); **←collapse/→expand** modern override + faithful `+`/`-`/`*` (AR-142); eager children, lazy deferred (AR-143); single-select, checkbox/multi deferred (AR-144). Reuses the RD-11 virtual-scroll helpers + owns a `ScrollBar` with a Tree-specific renderer (AR-145). New `src/tree/` (AR-148); additive `cpOutlineViewer` roles at plan GATE-1 (AR-149); kitchen-sink `Tree` story + `demo:tree` (AR-150). 13 AC. Next: `make_plan RD-15`. |
