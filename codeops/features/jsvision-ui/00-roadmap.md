...
> **Status**: In Progress
> **Created**: 2026-06-29
> **Last Updated**: 2026-07-01 (RD-11 containers/scrolling/lists **drafted** ✏️ — `add_requirement`, AR-103…AR-114)
> **Progress**: 7 / 11 done (RD-01 ✅, RD-02 ✅, RD-03 ✅, RD-04 ✅, RD-05 ✅, RD-06 ✅, RD-10 ✅) · RD-11 ✏️ Drafted
...
| RD-11 | Containers, scrolling & lists | [RD-11](requirements/RD-11-containers-scrolling-lists.md) | — | RD Drafted | ✏️ | 2026-07-01 | Phase 1 sibling of RD-06 (split per AR-93). **Drafted** ✏️ (`add_requirement`, 2026-07-01) — `ScrollBar` + `Scroller` (auto-owned bars) · generic single-column virtual-scroll `ListView<T>` (+ sorted/type-ahead · `ListBox` preset) · rich modal/modeless **`Dialog`** (hosts RD-06 controls via `execView`; terminating-command result + a child `valid()` close-gate = **DEF-16** · OK/Cancel/Yes/No helpers). Additive `cpScrollBar` (4–5) + ListViewer (26–29) core roles; kitchen-sink navigator upgrades to a `ListView` sidebar (dogfooding). 15 AC; **AR-103…AR-114** (8 user + 4 dominant). Deferred → RD-07: multi-column `ListViewer`/`Table`, `ComboBox`. Next: `make_plan RD-11` (TV decode `tscrlbar.cpp`/`tscrolle.cpp`/`tlistvie.cpp`/`tlistbox.cpp`/`tdialog.cpp`). |
...
- **2026-07-01** — **RD-11 (Containers, scrolling & lists) drafted** → stage `RD Drafted` ✏️.
  ...
  **15 AC; AR-103…AR-114** (AR-103…110 user, AR-111…114 dominant). Deferred → RD-07: multi-column
  ...
  the portfolio row.
- **Recommended next:** **`make_plan RD-11`** — produce the implementation plan spec-first, **reading each
  component's TV source first** per the fidelity directive (GATE 1 decode of `TScrollBar`/`TScroller`/
  `TListViewer`/`TListBox`/`TDialog` draw + geometry + `getColor` chain into the `03-NN-*.md` specs, plus
  the BEFORE/AFTER gate tasks in `99-execution-plan.md`); optionally `preflight` the plan first.
