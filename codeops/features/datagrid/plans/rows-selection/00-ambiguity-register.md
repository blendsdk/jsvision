...
| AR-10 | Selection survives sort/filter for free | Because selection is a **key set** (RD AR-15), it needs **no reconcile** on re-sort/re-filter — the keys are stable; the same rows re-highlight wherever they move. This **replaces** the vestigial single `selected` index **and its by-key re-anchor logic** (`grid.ts` `snapshotAnchors`/reconcile at `:886`–`:942`, which today only ever re-anchors `-1`). Deleting rows clears their keys from the set (AR-12); the anchor key is cleared if its row is gone. | ✅ Resolved |
