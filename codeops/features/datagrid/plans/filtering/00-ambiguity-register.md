...
| 27 | `03-03` specs the operator selector as "a small ComboBox/CheckGroup-style selector" (loose). `CheckGroup` is multi-select (wrong for one operator) and a `ComboBox` is a heavier dropdown. | The operator selector is a **`RadioGroup`** (single-select, inline — the `RadioGroup(number-signal)+Input` idiom from `theme-designer/inspector-panel`). `FilterPopup`'s public seam is `operators()` / `currentOperator()` / `selectOperator(op)` (op values) over the group's internal index signal, not a raw operator signal. Envelope forwarding (AR #18) still matters — the date operands' `DatePicker` opens its calendar through it. | (runtime) |
| 28 | `03-04` mounts the popup via `mountCellOverlay` (which provides no dismissal), yet `03-03`'s error table requires "Escape / click-away closes". | Escape is handled in `FilterPopup.onEvent` (focus-chain bubble); **click-away** is a container-level **`PopupCatcher`** — a transparent full-overlay `View` added below the popup whose outside mouse-down closes it and is consumed (the documented `dropdown/popup.ts` pattern). Satisfies the stated mount mechanism *and* the dismissal requirement. | (runtime) |
| 29 | `03-03`/`01-requirements` fix that the popup holds *both* a condition section and the value-list section, but not the concrete layout, sizing, or how each section applies. | Both sections stack vertically in one fixed-size popup (condition rows 0–7, value-list rows 8–16; `FILTER_POPUP_HEIGHT` grew 8→17). Each section keeps its own Apply (the condition emits its `ColumnFilter`; the value-list emits `{ kind: 'set' }`), last-writer-wins per AR #20. The value-list defaults every label checked (≡ no filter). The tall popup clips against a short grid — a v1 limitation (no popup scroll/flip until a later pass). Behavior is per spec; only the layout/sizing is this amendment. | (runtime) |
