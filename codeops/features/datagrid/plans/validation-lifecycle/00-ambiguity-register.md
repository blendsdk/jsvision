...
| # | Category | Decision | Grounding | Status |
|---|----------|----------|-----------|--------|
| AR-23 | Placement / guard (runtime) | **The `grid.ts` line guard re-bases to `< 1450`, not the plan's estimated `< 1350`.** AR-7 correctly predicted the guard crossing and mandated the re-base; its specific `< 1350` estimate proved low. `grid.ts` entered RD-12 at 1298, and the *documented public surface alone* (the `beforeSave`/`validateRow`/`status`/`emptyText` options with their mandatory JSDoc `@example`s, plus the thin `isInvalid`/`activeMessage` accessors) puts it at 1352 after Phase 2, with a projected ~1407 for the full feature. All three guard tests re-base `< 1300` → `< 1450` in lockstep, with the AR-7 rationale intact: heavy logic stays in the new modules (`error-registry.ts`/`validation.ts`/`grid-lifecycle.ts`), and the guard is never met by re-inlining. `< 1450` gives headroom for the full feature so no second re-base is needed. | grid.ts 1298 (pre-RD-12) → 1352 (post-Phase-2, measured); the three guards at `grid-selection.impl.test.ts`, `grid-footer.impl.test.ts`, `navigation.impl.test.ts` | ✅ |
