...
| 18 (runtime) | Integration / Feasibility (**surfaced during Phase 3 execution**) | Header clicks never reached `SortHeader`: the container's editor `overlay` (`grid.ts`, `position:'fill'`) resolves to the whole grid content box and, even when empty, is the top-most hit — a mouse-down bubbles up the overlay's own ancestors (never across to the header sibling), so it is swallowed. Sorting is the first mouse consumer in this container, so the latent block was never exercised (the read-only grid uses key input only) | A: make the empty overlay hit-transparent (hide it until it hosts an editor); B: constrain the overlay to only the body band; C: reorder z so the header sits above the overlay | **A** — an `EditorOverlay` (Group subclass) hides itself (`state.visible=false`) while it has no children and shows when an editor mounts; `topMost` skips a hidden subtree, so header/body clicks fall through. Datagrid-local (no ui change, honours AR #1). Clearly-correct latent-bug fix (an empty overlay must not eat clicks); full datagrid suite (183 tests, incl. all RD-02/03 editing) stays green. B/C rejected as more invasive restructuring | ✅ Resolved |
