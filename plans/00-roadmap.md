...
> **Created**: 2026-06-27
> **Last Updated**: 2026-06-28 (RD-09 plan preflighted — 7 findings resolved, cleared for execution)
...
| RD-09 | [Testing Strategy & Acceptance Gate](../requirements/RD-09-testing-and-acceptance.md) | all | 🔄 Executing | [rd-09-testing-and-acceptance](rd-09-testing-and-acceptance/00-index.md) |
...
- **RD-09** is **📋 Plan Created** (2026-06-28): plan at `plans/rd-09-testing-and-acceptance/` (15-item ambiguity register, ✅ gate passed; 6 phases / 34 tasks). Builds the missing tiers on Linux — input corpus (hex-in-JSON, Tier 1), golden-screen (`@xterm/headless`, Tier 2, all four depths), extended **no-node-pty** Tier-3 e2e, seeded fuzz harness, deterministic bytes∝damage benchmark — plus the go/no-go gate (`docs/acceptance-gate.md` + `npm run gate`). Existing 407 Tier-1 tests kept as-is and mapped (AR-8). Deferred: **DEF-1** 3-OS CI cells green (no remote) · **DEF-2** macOS/Windows acceptance + Tier-4 manual matrix · **DEF-3** real-PTY SIGWINCH resize · **DEF-4** wall-clock perf budgets (→RD-10). **🔬 Preflighted** (2026-06-28): 7 findings (4 major) resolved and applied — gate criterion numbering unified to canonical RD-09 1–11, render-API examples corrected (`set(x,y,char,style)`, no `clone()`), Tier-3 `mouse` override fixed to a `MouseCaps` object, corpus gained an `expectedQueries` channel + CPR scoped out. Report: `rd-09-testing-and-acceptance/00-preflight-report.md`. Next: execute via `exec_plan`.
