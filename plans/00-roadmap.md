...
> **Created**: 2026-06-27
> **Last Updated**: 2026-06-27 (RD-07 plan created)
...
| RD-07 | [Host & Lifecycle](../requirements/RD-07-host-and-lifecycle.md) | RD-02, RD-04, RD-06 | 📋 Plan Created | [rd-07-host-and-lifecycle](rd-07-host-and-lifecycle/00-index.md) |
...
- **RD-07** is **📋 Plan Created** (2026-06-27): native `tty` host under `src/engine/host/` (8 files: `types`, `host`, `modes`, `signals`, `streams`, `platform`, `restore`, `index`). Plan: 5 phases / 24 tasks. Public surface is a `createHost(options): Host` factory (AR-1) with typed callbacks (AR-2) and a `render(buffer)` that owns prev+serialize+write (AR-3). All OS effects sit behind an injectable `RuntimeAdapter` so behavior is tested in-process; one thin subprocess e2e proves the real SIGINT→exit wiring (AR-13). Zero-Ambiguity Gate: 17/17 resolved. **Windows** paths are implemented but acceptance is **deferred-to-Windows-runner** (AR-4) like RD-01's deferred-to-remote CI — RD-07 reaches **🔒 Verified** only once a Windows runner passes. Next: exec_plan RD-07, then RD-08.
