...
| PL-9 (runtime) | Tooling / Scope | `prettier --check .` flags every `plans/**` and `requirements/**` markdown doc; the 03-02 `.prettierignore` spec didn't anticipate Prettier running over the CodeOps docs tree. Format them, or exclude them? | A: format the docs (churns hand-built tables in the register/RDs) · B: add `plans` + `requirements` to `.prettierignore` (Prettier owns code, not process docs) | **B — exclude `plans/` and `requirements/` from Prettier.** Low-stakes, in-bounds scoping call made during Session 2.3 (task 2.3.2). These are authored process artifacts, not code; formatting would mangle their tables and pollute the Phase 2 diff. Source/config remain fully gated. | ✅ Resolved (runtime) |
