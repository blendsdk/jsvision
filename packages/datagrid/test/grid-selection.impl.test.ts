...
test('AR-6: the selection controller is extracted to grid-selection.ts and grid.ts delegates to it', () => {
  ...
  // grid.ts entered RD-08 already ~1029 lines; with the selection controller extracted it stays a thin
  // set of delegators. This ceiling is a runaway-growth guard, not the 700-line target (AR-6).
  const lineCount = grid.split('\n').length;
  expect(lineCount).toBeLessThan(1120);
});
