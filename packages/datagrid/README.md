...
Escape hint. With the body focused, Escape restores the value present before the first accepted
commit to each changed cell in that trapped row session. This is scoped row recovery, not a general
undo stack.
...
const grid = new EditableDataGrid<Line>({
  ...
  onRevertRow: async ({ rowKey, row, cells }) => {
    await persistRestoredRow(rowKey, row, cells);
    return true;
  },
});
```
...
immutable changed-cell list in first-commit order. Return `true` only after persistence accepts the
atomic restore. Return `false`, throw, or reject to compensate the row to its pre-revert committed
values. The trap remains available for retry only while the same row and session remain live and
compensation completes; stale settlement still compensates its captured row but cannot reattach a
retry session. While the callback settles, the grid blocks competing edits, navigation, filtering,
selection, and row mutations.
