...
/**
 * Apply a cell edit immediately, run the optional `onCommit` veto sink, and revert on veto.
 *
 * The write happens first (`apply(row, columnId, next)`), so the record and grid reflect the edit at
 * once. Then `onCommit` is awaited: on `true` the new value stays; on `false` or a rejected promise
 * the record is reverted with `apply(row, columnId, previous)`. With no `onCommit` the commit
 * succeeds. This is a single round-trip — serializing overlapping in-flight commits is a caller
 * concern.
 *
 * @param args The edit: the `row`, `columnId`, `rowKey`, the `previous`/`next` values, an `apply`
 *   writer that mutates the record, and an optional `onCommit` sink.
 ...
 *   onCommit: (c) => c.value >= 0, // accept non-negative balances
 * });
...
export async function commitCell<T, V>(args: {
  ...
  const { row, columnId, rowKey, previous, next, apply, onCommit } = args;
  apply(row, columnId, next); // immediate in-memory write
  if (!onCommit) return { committed: true, value: next };

  let accepted: boolean;
  try {
    accepted = await onCommit({ rowKey, columnId, value: next, previous, row });
  } catch {
    accepted = false; // a rejected commit is a veto, not a crash
  }
  if (accepted) return { committed: true, value: next };

  apply(row, columnId, previous); // revert
