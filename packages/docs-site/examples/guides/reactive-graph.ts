 ...
 */
import { Button, Group, Text, at, batch, computed, createRoot, effect, signal } from '@jsvision/ui';
  ...
  build: (ctx) =>
    createRoot((disposeReactive) => {
      ...
      const batchSale = new Button('~B~atch sale', {
        onClick: () => {
          const previousRuns = effectRuns.peek();
          batch(() => {
            price.set(9);
            quantity.set(3);
          });
          action.set(
            effectRuns.peek() === previousRuns
              ? 'batch: values unchanged, no effect rerun'
              : 'batch: price + quantity, one effect',
          );
        },
      });
      const reset = new Button('~R~eset', {
        onClick: () => {
          ...
          action.set(
            effectRuns.peek() === previousRuns
              ? 'reset: values unchanged, no effect rerun'
              : 'reset batched to one consistent snapshot',
          );
        },
      });
      ...
      content.add(
        at(new Text('Writable sources feed one lazy derived value and one imperative observer.'), 0, 0, 66, 1),
      );
