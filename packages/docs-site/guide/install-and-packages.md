...
| ------------------------------------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| Node rejects the engine requirement         | Node is older than 22                                        | Install Node 22 or newer and repeat the install                                   | `node --version` satisfies every package's `engines.node`                  |
...
| `ERR_REQUIRE_ESM`                           | CommonJS `require()` tried to load an ESM-only package       | Use `"type": "module"` and static `import`                                        | Node reaches application startup without the ESM error                     |
| `ERR_PACKAGE_PATH_NOT_EXPORTED`             | An internal or guessed subpath is outside the exports map    | Import a documented public entry point or exported subpath                        | The installed exports map contains the exact requested key                 |
