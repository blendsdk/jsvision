...
export class KanbanSparseHeightIndex {
  ...
  readonly #maximumAnchors: number;
  readonly #maximumRuns: number;
  readonly #revisions: KanbanSparseHeightSnapshot['revisions'];
   ...
   */
  measure(input: KanbanSparseHeightMeasurement): void {
    this.#active();
    let cardKey: CardKey;
    let properties: Readonly<Record<string, unknown>>;
    try {
      properties = snapshotKanbanDataProperties(input, MEASUREMENT_KEYS.size);
      validateKanbanDataKeys(properties, MEASUREMENT_KEYS);
  ...
  snapshot(): KanbanSparseHeightSnapshot {
    ...
    return Object.freeze({
      ...
      retainedRuns,
      allocatedEntries: this.#measurements.size + retainedRuns,
