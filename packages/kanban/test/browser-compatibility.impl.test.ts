...
/** Absolute source root inspected by this browser-portability regression test. */
...
/** Lists modules reachable from the production entry, excluding the isolated testing-only tree. */
function listSourceFiles(): readonly string[] {
  ...
  while (pending.length > 0) {
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && entry.name !== 'testing') pending.push(path);
      else if (entry.isFile() && path.endsWith('.ts') && entry.name !== 'testing.ts') files.push(path);
...
describe('browser compatibility implementation', () => {
  it('keeps Node built-ins out of every production-entry source module', () => {
