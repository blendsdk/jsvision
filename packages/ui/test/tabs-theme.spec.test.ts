...
test('ST-30: the tab* roles are the ONLY additive keys — every existing role is unchanged', () => {
  ...
  // The tab* roles are the ONLY new keys added to the theme (additive-only surface).
  const knownKeys = new Set([...Object.keys(EXPECTED_UNCHANGED), ...TAB_ROLES]);
  const actualKeys = Object.keys(defaultTheme);
  const unexpected = actualKeys.filter((k) => !knownKeys.has(k));
  expect(unexpected, 'no theme key beyond the pre-existing set + the 3 tab* roles').toEqual([]);
});
