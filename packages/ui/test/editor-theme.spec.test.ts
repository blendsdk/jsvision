...
test('ST-32: the editor-family roles are the ONLY additive keys — every existing role is unchanged', () => {
  ...
  const LATER_ADDITIVE_ROLES = ['sliderTrack', 'sliderThumb', 'dangerText', 'warningText'] as const; // Slider (slider-theme.spec) + severity text roles (severity-text-theme.spec)
