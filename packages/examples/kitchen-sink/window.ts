 ...
 * closable) and paints its frame in the grey `dialog` role — the `cpGrayDialog` habitat the RD-06
 * controls are calibrated for. The frame geometry replicates Turbo Vision `TFrame::draw`
 * (`magiblot/tvision` `source/tvision/tframe.cpp:35-124`): double-line border when active, a centered
 * title. (The `TDialog`-style grey frame is the same faithful shape used by `controls-live/dialog.ts`;
 * a framework `Dialog` is reserved for RD-11, so this stays a demo-local escape hatch.)
 *
...
/** Double-line border (CP437 0xC9/CD/BB/BA/C8/BC) — active/focused. */
const DOUBLE = { tl: '╔', tr: '╗', bl: '╚', br: '╝', h: '═', v: '║' } as const;
/** Single-line border (CP437 0xDA/C4/BF/B3/C0/D9) — inactive. */
...
export class StoryWindow extends Window {
  ...
  /** Paint the grey `TFrame` chrome (double border when active, centered title; no close box). */
 ...
 * `kitchen.*` navigation). It never draws; a routed `CommandEvent` whose name has a handler is
